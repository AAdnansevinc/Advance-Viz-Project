...
setwd(dirname(rstudioapi::getSourceEditorContext()$path))   # Set the current path as directory.
...
names(data_raw)<-c("Country",                       # Rename Columns
               "Year", 
               "HappinessScore",
               "GDPPer",
               "SocialSupport",
...
data$Negative.affect<-NULL     # Drop Column
data$Positive.affect<-NULL     # Drop Column
data$Confidence.in.national.government<-NULL   # Drop Column

names(data)

#Rename Columns
 
names(data)<-c("Country",          # Rename Columns
               "Year", 
               "HappinessScore",
               "GDPPer",
               "SocialSupport",
...
ggplot(data_hist %>% filter(continent != "Seven seas (open ocean)"), aes(x = HappinessScore, fill = continent)) +  # Filter out Open seas.
  ...
  facet_wrap(~ continent) + guides(fill = FALSE)+                       # in order to separate via  continent.
